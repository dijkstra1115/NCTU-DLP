...
public:
	...
	/**
	 * calculate possible probability of all empty tile
	 * 在這邊我要把空tile跟num的資訊回傳到select_baset_action，然後（estimate所有可能的棋盤結果）＊（出現機率）
	 * 所以可能的棋盤可以用raw來表示，不過要記得把原始的raw記錄下來以便還原。
	 */
	int allpopup() {
		int space[16], empty_num = 0;
		for (int i = 0; i < 16; i++)
			if (at(i) == 0) {
				space[empty_num++] = i;
			}
	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				...
				int probability_value = 0;
				for (int i = 0; i < num; i++){
					board* popup_board = new board(uint64_t(after_state));
					popup_board->set(space[i], 1);
					probability_value += 0.9f * estimate(*popup_board) / num;
					popup_board->set(space[i], 2);
					probability_value += 0.1f * estimate(*popup_board) / num;
					delete popup_board;
				}
				move->set_value(move->reward() + probability_value);
...
int main(int argc, const char* argv[]) {
	...
	tdl.add_feature(new pattern({ 0, 1, 2, 5, 9, 13 }));
