...
def parse_args():
    ...
    parser.add_argument('--model_dir', default='./logs/fp/rnn_size=256-predictor-posterior-rnn_layers=2-1-n_past=2-n_future=10-lr=0.0020-g_dim=128-z_dim=64-last_frame_skip=False-beta=0.0001000/continued'\
...
def train(x, cond, modules, optimizer, args, device, kl_anneal):
    x = x.to(device)
    cond = cond.to(device)
...
class kl_annealing():
    def __init__(self, args):
        ...
        self.period = args.epoch_size // args.kl_anneal_cycle
    ...
    def get_beta(self):
        if (self.epoch % self.period) < self.period-1:
            self.beta += 1 / (self.period * self.ratio)
        else:
...
def main():
    ...
    for epoch in range(start_epoch, start_epoch + niter):
        ...
        for _ in tqdm(range(args.epoch_size)):
            ...
            seq = seq[:args.n_past+args.n_future]
            cond = cond[:args.n_past+args.n_future]
            loss, mse, kld = train(seq, cond, modules, optimizer, args, device, kl_anneal)
            epoch_loss += loss
            epoch_mse += mse
            epoch_kld += kld
        ...
        # record loss for drawing the curve
        loss_record=[epoch_loss  / args.epoch_size, \
                    epoch_mse / args.epoch_size, \
                    epoch_kld / args.epoch_size]
        loss_records.append(loss_record)
        
        '''
        Perhaps we don't need this evaluation step.
        If we really need evaluation step,
        maybe we need to calculate the loss and psnr instead of generate image.
        '''
        ...
        if epoch % 5 == 0:
            ...
            for _ in range(len(validate_data) // args.batch_size):
                ...
                validate_cond = validate_cond[:args.n_past+args.n_future]
                # Basically this line is the evaluation step.
                pred_seq = pred(validate_seq, validate_cond, modules, args, device)
                
                # We still need to evaluate the psnr based on the pred_seq.
                # But we don't have to plot the image.
                # if epoch % 20 == 0:
                #     plot_pred(pred_seq, args)
            ...
            # record psnr for drawing the curve
            psnr_record.append([ave_psnr])
    ...
    # drawing the curve
    ipdb.set_trace()
    loss_records.to_csv('./loss_records.csv', index=False)
    psnr_record.to_csv('./psnr_record.csv', index=False)
    draw_curve(niter, loss_records, psnr_record)
